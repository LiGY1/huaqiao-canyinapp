...
function useChatHistory(messages, conversationId) {
  ...
  const saveChatToDatabase = async (
    currentUserMessageValue,
    currentAIResponseValue,
    currentUserTimestampValue,
    currentAIFilesValue,
  ) => {
...
function useChatStream(messages, conversationId, isTyping, scrollToBottom, resetAutoScroll, saveChatToDatabase) {
  ...
  const handleSendMessage = async (text) => {
    ...
    const inputs = {
      ...
      student_number: firstChild.studentId,
    };
    // ---------------------------------------
    try {
      await sendMessage(
...
const { handleSendMessage, sendQuickMessage } = useChatStream(
  messages,
  conversationId,
  isTyping,
  scrollToBottom,
