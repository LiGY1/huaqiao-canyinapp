<template>
  <view :id="attrs.id" :class="'_block _'+name+' '+attrs.class" :style="attrs.style">
    <block v-for="(n, i) in nodes" v-bind:key="i">
      ...
      <rich-text v-else-if="!n.c&&n.attrs&&!handler.isInline(n.name, n.attrs.style)" :id="n.attrs.id" :style="n.f" :user-select="opts[4]" :nodes="[n]" />
      <!-- #endif -->
...
export default {
  ...
  watch: {
    childs: {
      handler (nodes) {
        // defensive: ensure nodes is an array (watcher can be called with undefined)
        nodes = nodes || [];
        // 列表缩短会刷新整个列表，因此进行空填充
